 ...
 * @file
 * Drawtext Filter.
 * Based on the original FFmpeg vhook/drawtext.c filter by Gustavo
 * Sverzut Barbieri.
 */
...
#define GET_PIXEL(picref, yuv_color, x, y, hsub, vsub) { \
    yuv_color[0] = picref->data[0][( x           ) + ( y           ) * picref->linesize[0]]; \
    yuv_color[1] = picref->data[1][((x) >> (hsub)) + ((y) >> (vsub)) * picref->linesize[1]]; \
    yuv_color[2] = picref->data[2][((x) >> (hsub)) + ((y) >> (vsub)) * picref->linesize[2]]; \
}
    ...
    if (bitmap->pixel_mode == ft_pixel_mode_mono) {
        ...
        for (r=0; (r < bitmap->rows) && (r+y < height); r++) {
            for (c=0; (c < bitmap->width) && (c+x < width); c++) {
                ...
                GET_PIXEL(picref, dpixel, c+x, y+r, hsub, vsub);
                ...
                if (outline) {
                    ...
                    if (!in_glyph && spixel) {
                        ...
                        if (c-1 >= 0)
                            SET_PIXEL(picref, yuv_bgcolor, c+x-1, y+r, hsub, vsub);
                    ...
                    /* see if we have a top/bottom border */
                        ...
                        if ((r-1 >= 0) && (!(bitmap->buffer[(r-1) * bitmap->pitch + c/8] & (0x80>>(c%8)))))
                            ...
                            SET_PIXEL(picref, yuv_bgcolor, c+x, y+r-1, hsub, vsub);
                ...
                SET_PIXEL(picref, dpixel, c+x, y+r, hsub, vsub);
            }
        }
    }
}
...
static void draw_text(AVFilterContext *ctx, AVFilterBufferRef *picref, int width, int height)
{
    DrawTextContext *dtext = ctx->priv;
...
static void end_frame(AVFilterLink *link)
{
    ...
    AVFilterBufferRef *picref = link->cur_buf;

    draw_text(link->dst, picref, picref->video->w, picref->video->h);

    avfilter_draw_slice(output, 0, picref->video->h, 1);
